...
bool sm3t__set_conf_value(lua_State *L, const char *path, sm3t_type_t type, sm3t_value_t *in) {
    ...
    switch (type) {
        case SM3T_CINT:    lua_pushinteger(L, in->as._int); break;
        case SM3T_CFLOAT:  lua_pushnumber(L, in->as._float); break;
        case SM3T_CBOOL:   lua_pushboolean(L, in->as._bool); break;
        case SM3T_CSTRING: lua_pushstring(L, in->as._string); break;
        default:
