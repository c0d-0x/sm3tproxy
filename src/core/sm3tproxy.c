...
int main(int argc, char *argv[]) {
    ...
    if ((L = sm3t__init_conf("examples/conf.lua")) == NULL) return EXIT_FAILURE;
    if (*port > UINT16_MAX || *port <= 0) {
        free_args(&cmd_arg);
        SM3T__FATAL("Invalid port number");
    }

    if ((in_lua.as._int = (int64_t) *port) != 0) sm3t__set_conf_value(L, "tcp.listen.port", SM3T_CINT, &in_lua);

    in_lua.as._int = (int64_t) 0;
    in_lua.as._bool = *debug;
